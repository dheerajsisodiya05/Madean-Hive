...
export const fetchUserOrderHistory = createAsyncThunk<Order[], string, { rejectValue: string }>(
...
>(
  ...
  async ({ address, jwt, paymentGateway }, { rejectWithValue }) => {
    try {
      // Convert payment gateway name to uppercase enum format (RAZORPAY or STRIPE)
      const paymentMethod = paymentGateway.toUpperCase();
      
      const response = await api.post<Record<string, unknown>>(API_URL, address, {
        headers: { Authorization: `Bearer ${jwt}` },
      ...
      if (response.data?.payment_link_url) {
        if (paymentGateway === "Razorpay") {
          window.location.href = response.data.payment_link_url as string;
  ...
  async ({ paymentId, jwt, paymentLinkId }, { rejectWithValue }) => {
    try {
      ...
      if (axios.isAxiosError(error) && error.response) {
        ...
        return rejectWithValue((errorData?.message as string) || "Failed to process payment");
      }
  ...
  async (orderId, { rejectWithValue }) => {
    try {
      ...
      if (axios.isAxiosError(error) && error.response) {
        const errorData = error.response.data as Record<string, unknown>;
        return rejectWithValue(
          (errorData?.error as string) || "An error occurred while cancelling the order."
        );
      }
...
const orderSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(createOrder.fulfilled, (state, action: PayloadAction<Record<string, unknown>>) => {
