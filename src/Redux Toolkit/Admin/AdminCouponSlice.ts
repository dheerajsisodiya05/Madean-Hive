...
>("coupon/createCoupon", async ({ coupon, jwt }, { rejectWithValue }) => {
  try {
    ...
    return rejectWithValue(axiosError.response?.data?.message || "Failed to create coupon");
  }
});
...
>("coupon/deleteCoupon", async ({ id, jwt }, { rejectWithValue }) => {
  try {
    ...
    const axiosError = error as AxiosError<{ message: string }>;
    return rejectWithValue(axiosError.response?.data?.message || "Failed to delete coupon");
  }
});
...
>("coupon/fetchAllCoupons", async (jwt, { rejectWithValue }) => {
  try {
    ...
    const axiosError = error as AxiosError<{ message: string }>;
    return rejectWithValue(axiosError.response?.data?.message || "Failed to fetch coupons");
  }
});
